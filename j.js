...
$(function () {
    document.getElementById("selectLayer").onchange = function () {
        ...
        $(document).ready(function () {
            $.ajax({
                type: "GET",
                url: "http://localhost:8080/geoserver/wms?service=WFS&request=DescribeFeatureType&version=1.1.0&typeName=" + value_layer,
                dataType: "xml",
                success: function (xml) {
    ...
    document.getElementById('attQryRun').onclick = function () {
        ...
        if (layer.options.selectedIndex == 0) {
            ...
            var url = "http://localhost:8080/geoserver/Practice/wms?service=WMS&version=1.1.0&request=GetFeature&typeName=" + value_layer + "&CQL_FILTER=" + value_attribute + "+" + value_operator + "+'" + value_txt + "'&outputFormat=application/json"
            // console.log(url);
